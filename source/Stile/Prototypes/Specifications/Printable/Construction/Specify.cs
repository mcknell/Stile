...
namespace Stile.Prototypes.Specifications.Printable.Construction
{
	public static class Specify
	{
		...
		{
			return new PrintableSubjectBuilder<TSubject>(new PrintableSource<TSubject>(expression));
		}

		[Pure]
		public static IPrintableBoundSubjectBuilder<TSubject> For<TSubject>(TSubject subject) where TSubject : class
		{
			return new PrintableSubjectBuilder<TSubject>(new PrintableSource<TSubject>(subject));
		}
		...
		[Pure]
		public static IFluentSpecificationBuilder<TSubject> ThatAny<TSubject>()
		{
			return new PrintableSpecificationBuilder<TSubject>(x => x);
		}
	}
}
