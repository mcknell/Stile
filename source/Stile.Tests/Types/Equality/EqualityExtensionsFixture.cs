...
namespace Stile.Tests.Types.Equality
{
	...
	public class EqualityExtensionsFixture
	{
		...
		private static void EqualsOrIsEquallyNull_ForReferenceType<TValue>(TValue first, TValue second)
			where TValue : class
		{
			EqualsOrIsEquallyNull(first, second);
		}

		private static void EqualsOrIsEquallyNull_ForValueType<TValue>(TValue first, TValue second)
			where TValue : struct
		{
			EqualsOrIsEquallyNull(first, second);
