...
namespace Stile.Tests.Prototypes.Specifications.Printable.DSL.ExpressionBuilders.ResultIs
{
	...
	public class IsExtensionsFixture
	{
		...
		public void ComparablyEquivalentTo()
		{
			...
			AssertEquals(Specify.ForAny<DateTime>().That(x => x.Ticks).Is.Not.ComparablyEquivalentTo(0),
				Outcome.Succeeded,
				now);
		}
		...
		public void Null()
		{
			...
			const int one = 1;
			IPrintableSubjectBuilder<int?> printableSubjectBuilder = Specify.ForAny<int?>();
			IFluentSpecificationBuilder<int?, int?> fluentSpecificationBuilder = printableSubjectBuilder.That(x => x);
			INegatableIs<int?, int?, IIs<int?, int?, IFluentSpecification<int?, int?>>, IFluentSpecification<int?, int?>>
				negatableIs = fluentSpecificationBuilder.Is;
			AssertEquals(negatableIs.Null(), Outcome.Failed, one);
		...
		internal static void AssertEquals<T1, T2>(IFluentSpecification<T1, T2> specification,
			Outcome outcome,
			T1 value,
			string message = null)
		{
			...
			if (message != null)
			{
				Assert.That(evaluation.Emitted.Retrieved.Value, Is.EqualTo(message));
			}
		}
	}
}
