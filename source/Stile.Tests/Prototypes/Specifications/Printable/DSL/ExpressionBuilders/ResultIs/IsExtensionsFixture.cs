...
namespace Stile.Tests.Prototypes.Specifications.Printable.DSL.ExpressionBuilders.ResultIs
{
    ...
    public class IsExtensionsFixture
    {
        ...
        internal static void AssertEquals<T1, T2>(IPrintableSpecification<T1, T2> specification, Outcome outcome, T1 value, string message = null)
        {
            ...
            if (message != null)
            {
                Assert.That(evaluation.Emitted.Retrieved.Value, NUnit.Framework.Is.EqualTo(message));
            }
        }
    }
}
